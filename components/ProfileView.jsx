...
export default function ProfileView() {
  const { user, exchangeRate, completedQuests, quests } = useStore()
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex items-center gap-4 relative z-10">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs mt-0.5 truncate" style={{ color: 'rgba(255,255,255,0.45)' }}>
              {currentUser.email || 'ou.edu'}
            </p>
            <div className="flex items-center gap-1.5 mt-2 flex-wrap">
              <span className="badge badge-brand text-[9px]">★ {currentUser.points?.toLocaleString()} pts</span>
              <span className="badge badge-sakura text-[9px]">🇯🇵 Study Abroad</span>
            </div>
          </div>
        </div>
        ...
        <div className="mt-4 pt-4 relative z-10" style={{ borderTop: '1px solid rgba(255,255,255,0.08)' }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-display font-bold text-white text-sm">{currentUser.university || 'University of Oklahoma'}</p>
              <p className="text-[10px]" style={{ color: 'rgba(255,255,255,0.4)' }}>
                {currentUser.major || 'Marketing'} · {currentUser.year || 'Junior'} · Class of 2026
              </p>
            </div>
          </div>
